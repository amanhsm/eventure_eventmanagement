...
import { Calendar, Clock, Users, MapPin, Plus, ArrowLeft, Save } from "lucide-react"
...
export default function EventCreationForm() {
  ...
  // Step 1: Venue and Date
  const [selectedVenue, setSelectedVenue] = useState<Venue | null>(null)
  const [eventDate, setEventDate] = useState<Date | null>(null)
  const [startTime, setStartTime] = useState("")
  const [endTime, setEndTime] = useState("")
  
  // Step 2: Event details with all fields
  ...
  // Fetch organizer ID and categories on component mount
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        if (organizerData) {
          ...
          // Pre-fill contact person with organizer name
          setFormData(prev => ({
            ...prev,
  ...
  const handleSubmit = async (isDraft: boolean = false) => {
    ...
    try {
      ...
      const eventData = {
        ...
        // Populate all the missing fields
